...
export default function OperationalQueue({ queue = [] }) {
  ...
  const counts = useMemo(() =>
    TABS.reduce((acc,tab)=>{ acc[tab]=queue.filter(r=>r.status===tab).length; return acc; }, {}),
    [queue]
  );
  const filteredData = useMemo(()=>queue.filter(r=>r.status===activeTab),[queue,activeTab]);
        ...
        return (
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {ok && <span className="flex items-center gap-1 text-xs font-semibold text-green-600"><CheckCircle size={11}/> Assigned</span>}
            </div>
            <div className="flex items-center gap-1.5 mt-1 text-xs text-slate-400 flex-wrap">
              ...
              <span>•</span><span>{row.source}</span>
              <span>•</span><span className="flex items-center gap-0.5"><span>🕐</span>{row.elapsed}</span>
            </div>
          </div>
        );
      },
    ...
    columnHelper.accessor("id",{
      ...
      cell: (info) => {
        ...
        if (row.itemType === "tour") {
          return (
            <button
              onClick={() => navigate('/admin/tour-requests', { state: { tourId: row.rawId } })}
  ...
  const table = useReactTable({ data:filteredData, columns, getCoreRowModel:getCoreRowModel(), getFilteredRowModel:getFilteredRowModel() });

  return (
    <>
      {assignItem && (
        <AssignModal item={assignItem} onClose={()=>setAssignItem(null)}
          onSuccess={()=>{ setSuccessItem(assignItem.id); setTimeout(()=>setSuccessItem(null),3000); }}/>
      )}
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div className="overflow-y-auto max-h-[320px]">
          {table.getRowModel().rows.length===0 ? (
            <div className="px-6 py-10 text-center text-slate-400 text-sm">No {activeTab} items</div>
          ) : (
                ...
                {table.getRowModel().rows.map(row=>(
                  <tr key={row.id} className="hover:bg-slate-50 transition-colors cursor-pointer">
                    {row.getVisibleCells().map(cell=>(
                      <td key={cell.id} className="px-6 py-4" style={cell.column.columnDef.size?{width:cell.column.columnDef.size}:{}}>
