...
import { MoreHorizontal, Home, User, Calendar, Clock, Activity, Trash2, AlertTriangle } from "lucide-react";
import { MOCK_MODE, fetchPropertyDetail, deleteProperty } from "../api/propertiesApi";
    ...
    >
      ...
      >
        ...
        <div style={{ padding: "20px 20px 0", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
          ...
          <p style={{
            margin: "0 0 6px", fontSize: 13, fontWeight: 700, color: "#0f172a",
            background: "#f8fafc", border: "1px solid #e2e8f0",
            borderRadius: 8, padding: "6px 14px", maxWidth: "100%",
        ...
        <div style={{ padding: 20, display: "flex", gap: 10 }}>
          ...
          <button
            ...
            style={{
              flex: 1, padding: "11px", borderRadius: 10,
              border: "none", background: isDeleting ? "#f87171" : "#dc2626",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 16, paddingBottom: 20, borderBottom: "1px solid #f1f5f9" }}>
        {[
          ...
          { label: "Intent",     value: property.intent     ?? "-" },
          { label: "Visibility", value: property.visibility ?? "Public" },
        ].map(({ label, value }) => (
          <div key={label}>
      ...
      <div style={{ display: "flex", gap: 28, flexWrap: "wrap" }}>
        {[
          `${property.bedrooms ?? property.beds ?? 0} Bedrooms`,
          `${property.bathrooms ?? property.baths ?? 0} Bathrooms`,
          `${property.area ?? 0} sqft`,
        ].map((label) => (
          <span key={label} style={{ fontSize: 13, color: "#475569", fontWeight: 600 }}>{label}</span>
        ))}
      </div>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {property.amenities.map((a) => (
              <span key={a} style={{ fontSize: 12, padding: "4px 14px", borderRadius: 99, background: "#f1f5f9", color: "#475569", border: "1px solid #e2e8f0", fontWeight: 500 }}>{a}</span>
            ))}
          </div>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {property.highlights.map((h) => (
              <span key={h} style={{ fontSize: 12, padding: "4px 14px", borderRadius: 99, background: "#fef9c3", color: "#854d0e", border: "1px solid #fde68a", fontWeight: 600 }}>{h}</span>
            ))}
          </div>
        </div>
      )}
...
function EmptyView({ onBack }) {
  return (
    <div style={{ padding: "24px 28px", minHeight: "100%", background: "#f8fafc", fontFamily: "system-ui,sans-serif" }}>
      ...
      <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 20, textAlign: "center", color: "#64748b", fontSize: 14 }}>
...
export default function PropertyDetailPage({ propertyId, property: initialProperty, onBack, onEditProperty }) {
  ...
  const fallbackProperty = useMemo(() => {
    ...
    return {
      ...initialProperty,
      images:    Array.isArray(initialProperty.images)    ? initialProperty.images    : [],
      currency:  initialProperty.currency  || "USD",
      intent:    initialProperty.intent    || "For Sale",
      documents: Array.isArray(initialProperty.documents) ? initialProperty.documents : [],
    };
  }, [initialProperty]);
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 24 }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 6 }}>
              ...
              {property.featured && (
                <span style={{ fontSize: 11, fontWeight: 700, padding: "3px 11px", borderRadius: 99, background: "#fef9c3", color: "#854d0e" }}>Featured</span>
              )}
            </div>
        ...
        {onEditProperty ? (
          <PropertyActionsMenu
            ...
            onDeleteProperty={() => setShowDeleteModal(true)}
          />
        ) : (
          <button style={{ padding: "7px 9px", border: "1px solid #e2e8f0", borderRadius: 9, background: "#fff", cursor: "pointer", color: "#64748b", display: "flex", alignItems: "center" }}>
      ...
      <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", overflow: "hidden" }}>
            <div style={{ display: "flex", borderBottom: "1px solid #f1f5f9", padding: "0 8px", background: "#fafafa" }}>
              {TABS.map((tab) => (
                <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: "13px 18px", background: "transparent", border: "none", cursor: "pointer", fontSize: 13, fontWeight: activeTab === tab ? 700 : 500, color: activeTab === tab ? "#0f172a" : "#94a3b8", borderBottom: activeTab === tab ? "2px solid #22225E" : "2px solid transparent", marginBottom: -1, transition: "all 0.15s" }}>
                  {tab}
                </button>
              ))}
            </div>
        ...
        <div style={{ width: 300, flexShrink: 0, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", padding: 24 }}>
            <p style={{ margin: "0 0 4px", fontSize: 28, fontWeight: 900, color: "#0f172a", letterSpacing: "-0.5px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", padding: 20 }}>
            <p style={{ margin: "0 0 16px", fontSize: 13, fontWeight: 700, color: "#374151", display: "flex", alignItems: "center", gap: 7 }}>
              <Home size={14} color="#64748b" /> Property Info
            </p>
            {[
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", padding: 20 }}>
            <p style={{ margin: "0 0 16px", fontSize: 13, fontWeight: 700, color: "#374151", display: "flex", alignItems: "center", gap: 7 }}>
              <Activity size={14} color="#64748b" /> Activity
            </p>
            {activity.length === 0
