...
export default function LeadDetailView({ lead: initialLead, onBack }) {
  ...
  const priorityMutation = useMutation({
    ...
    onError: (err) =>
      showToast(
        'error',
        err?.response?.data?.message || 'Could not update priority',
      ),
  });
  ...
  const archiveMutation = useMutation({
    ...
    onError: (err) =>
      showToast(
        'error',
        err?.response?.data?.message || 'Could not update archive status',
      ),
  });
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  opacity: priorityMutation.isPending ? 0.6 : 1,
                }}
              >
                <svg
                  width="11"
      ...
      <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
        ...
        >
          <QuickActionsCard
            ...
            priStyle={priStyle}
            onStatusChange={(s) => {
              if (s !== lead.status) statusMutation.mutate(s);
            }}
...
function NotesTab({ lead, showToast }) {
  ...
  const addNoteMutation = useMutation({
    ...
    onError: (err) =>
      showToast('error', err?.response?.data?.message || 'Could not add note'),
  });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      >
        <button
          ...
          style={{
            ...
            fontSize: 13,
            fontWeight: 600,
            opacity: addNoteMutation.isPending ? 0.7 : 1,
          }}
        >
      ...
      {isNotesLoading ? (
        ...
        notes.map((n, i) => (
          ...
          >
            <p style={{ margin: '0 0 5px', fontSize: 13, color: '#374151' }}>
              {n.content || n.text}
            </p>
            <p style={{ margin: 0, fontSize: 11, color: '#94a3b8' }}>
              {timeAgo(n.createdAt)}
...
function ActivityTab({ lead }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      {[
        {
          text: `Lead created from ${lead.source ?? 'Website'}`,
          time: lead.createdAt,
          icon: '✨',
          color: '#6366f1',
        },
        {
          text: `Priority set to ${lead.priority}`,
          time: lead.createdAt,
          icon: '🚩',
          color: '#f97316',
        },
    ...
    >
      ...
      <div style={{ position: 'relative', marginBottom: 10 }}>
        <select
          ...
          disabled={isStatusLoading || isArchived}
          style={{
            width: '100%',
            ...
            color: '#374151',
            background: '#fff',
            cursor: isStatusLoading || isArchived ? 'not-allowed' : 'pointer',
            outline: 'none',
            appearance: 'none',
            opacity: isStatusLoading || isArchived ? 0.5 : 1,
          }}
        >
      ...
      <button
        ...
        style={{
          width: '100%',
          padding: '10px 14px',
          border: `1px solid ${isArchived ? '#bae6fd' : '#e2e8f0'}`,
          borderRadius: 9,
          fontSize: 13,
          fontWeight: 600,
          color: isArchived ? '#0369a1' : '#64748b',
          background: isArchived ? '#f0f9ff' : '#f8fafc',
          cursor: isArchiveLoading ? 'not-allowed' : 'pointer',
          display: 'flex',
          alignItems: 'center',
          gap: 8,
          opacity: isArchiveLoading ? 0.6 : 1,
        }}
      >
        {isArchived ? (
          <svg
  ...
  return (
    ...
    >
      ...
      {[
        ...
        { label: 'Updated', value: timeAgo(lead.createdAt), icon: 'Clock' },
